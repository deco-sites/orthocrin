      ...
      <div style={grid}>
        {gridItem.map((item, idx) => (
          <div style={gridItemStyle[idx]} class="relative">
            <img class="w-full h-full object-cover rounded-b-[10px] rounded-t-[10px]" src={item.imageMobile} alt={item.title} />
            <div class="absolute flex w-full justify-center bottom-[10px]">
              <a class="w-[165px] items-center flex justify-center h-[30px] rounded-b-[75pt] rounded-t-[75pt] bg-[#ed1c24d9] text-[#fff]" href={item.link}>{item.title}</a>
            </div>
          </div>
        ))}
      </div>
      <a href={allCategories} class="bg-[#000] w-full text-[#fff] flex justify-center rounded-b-[10px] rounded-t-[10px] text-[1.25rem] py-[12px] mt-[40px]">Ver todos os produtos</a>
    </div>
  );
}
